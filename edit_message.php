...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = $conn->prepare(
        "UPDATE `messages` SET `author`=?, `title`=?, `body`=? WHERE id=$messageId"
    );
...
<form class="m-4 w-50" action="edit_message.php?id=<?= $messageId; ?>"
      method="POST">
    <label class="form-label" for="author">Автор:</label>
    <input class="form-control" type="text" name="author" id="author"
           value="<?= $message['author']; ?>" required><br>

    <label class="form-label" for="title">Заголовок сообщения:</label>
    <input class="form-control" type="text" name="title" id="title"
           value="<?= $message['title']; ?>" required><br>

    <label class="form-label" for="full_content">Полное содержание:</label>
    <textarea class="form-control" name="full_content" id="full_content"
              rows="4" required><?= $message['body']; ?></textarea><br>

    <button class="btn btn-lg btn-primary" type="submit">Сохранить изменения
    </button>
</form>
